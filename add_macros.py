...
def before_build_macros(source, target, env):
    ...
    cprint("Welcome, this is the more secure macro input panel.\n")
    cprint(
    ...
    cprint('or type "n" , otherwise type anything else: \t')
        ...
        if read:
            ...
            if not os.path.isfile(filenme):
                if default_filename == filenme:
                    print(
                        bcolors.FAIL
                        + " [Error] Standart file dos not excist yet"
                        + bcolors.ENDC
                    )
                else:
                    print(
                        bcolors.FAIL
                        + " [Error] Please Specify Valid Input File"
                        + bcolors.ENDC
                    )
                exit(0)
            print(
                "give the password for the file [correctness will not be checked]",
                flush=True,
            )
            password = getpass()
